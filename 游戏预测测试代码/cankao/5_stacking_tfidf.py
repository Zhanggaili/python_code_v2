...
ada_model_path = common_path + r'/data/model/ada_model.m'
ET_model_path = common_path + r'/data/model/ET_model.m'
...
def build_model_save(X, Y):
    ...
    # Ada堆叠
    print('Ada begin ')
    model_Ada = AdaBoostClassifier(DecisionTreeClassifier(max_depth=5), algorithm="SAMME", n_estimators=10)
    model_Ada.fit(X,Y)
    joblib.dump(model_Ada, ada_model_path)


    # ET堆叠
    print('ET begin ')
    model_ET = ExtraTreesClassifier(n_estimators=100, max_features='auto' , max_depth=6, n_jobs=-1)
    model_ET.fit(X,Y)
    joblib.dump(model_ET, ET_model_path)
...
def stacking_feat(tfidf_data):
    ...
    model_Ada = joblib.load(ada_model_path)
    model_ET = joblib.load(ET_model_path)
    ...
    y1_Ada = model_Ada.predict_proba(tfidf_data)
    y1_Ada_df = pd.DataFrame(y1_Ada, columns=['Ada','Ada1'])
    ...
    y1_ET = model_ET.predict_proba(tfidf_data)
    y1_ET_df = pd.DataFrame(y1_ET, columns=['ET0','ET1'])


    return pd.concat([y1_lr_df,
                      # y1_svm_df,
                      y1_rf_df,
                      # y1_bayes_df,
                      # y1_Ada_df,
